		...
		const uint16_t vids[], const uint16_t pids[])
{
	for (unsigned i = 0; vids[i] && pids[i]; i++) {
		LOG_INFO("Reading USB idVendor %x, idProduct %x", dev->descriptor.idVendor, dev->descriptor.idProduct);
		if (dev->descriptor.idVendor == vids[i] &&
			dev->descriptor.idProduct == pids[i])
		{
			LOG_INFO("Found USB idVendor %x, idProduct %x", dev->descriptor.idVendor, dev->descriptor.idProduct);
			return true;
		}
	}
	return false;
